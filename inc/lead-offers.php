...
class LeadOffers {
    ...
    public function offer_meta_box_callback( $offer ) {
        ...
        ?>
            ...
            <input type="hidden" name="offer_meta_fields_nonce" value="<?php echo wp_create_nonce(LEAD_CONST['plugin_basename']); ?>" />
        <?php
    }

    public function save_meta_box_for_offers($offer_id){
        // проверяем nonce нашей страницы, потому что save_post может быть вызван с другого места.
        if ( ! wp_verify_nonce( $_POST['offer_meta_fields_nonce'], LEAD_CONST['plugin_basename'] ) )
            return;
    ...
    */
    ...
    private function download_all_cats($limit = 50, $offset = 0) {
        ...
        $count_loading_cats = count($answer['data']);
        $this->loading_cats = $answer['data'];

        while ($limit == $count_load_cats){
            $offset += 60;
            $answer = $this->download_all_cats($limit, $offset);
            $count_loading_cats = count($answer['data']);
            array_merge($this->loading_cats, $answer['data']);
        }
    ...
    public function recreate_cats($name = '') {
        ...
        foreach ($this->loading_cats as $for_create_cat){
            ...
            if( ! is_wp_error($result_ins) ) {
                ...
                $result['loading_cats_id'] = $loading_cats_id;
    ...
    */
    ...
    //     $download_offer = json_decode( wp_unslash( $_POST['offer'] ));
    ...
    //                 'key'   => 'id_lead',
    //                 'type'  => 'NUMERIC',
    //                 'meta_value_num'    => $download_offer->id
    //             ]
    //         ]
    //     ];

    //     $local_offer = get_posts( $atts );

    //     if (count($local_offer)) {
    //         foreach ($local_offer as $current_offer) {
    //             setup_postdata( $current_offer );
    //             // $result['current_offer'] = $current_offer;
    //             // $status = $current_offer->post_status;
    //             $status = get_post_status($current_offer->ID);
    //             if ($status != 'publish') {
    //                 $atts = [
    //                     'ID'            => $current_offer->ID,
    //                     'post_status'   => 'publish'
    //                 ];
    //                 $result_update = wp_update_post( wp_slash( $atts ) );
    //                 if (is_wp_error( $result_update )) {
    //                     $result['message'] = "{$download_offer->site_name} -- Ошибка обновления записи";
    //                     $result['err'] = $result_update->get_error_message();
    //                     $result['result'] = false;
    //                     echo json_encode($result);
    //                     return;
    //                 };
    //                 $result['message'] = "{$download_offer->site_name} -- оффер обновлен";
    //             } else {
    //                 $result['message'] = "{$download_offer->site_name} -- оффер  не нуждается в обновлении";
    //             };
    //         };
    //     } else {
    //         $result_ins = $this->insert_offer($download_offer);
    //         if (is_wp_error($result_ins)) {
    //             $result['message'] = "{$download_offer->site_name} -- Ошибка добавления оффера";
    //             $result['err'] = $result_ins->get_error_message();
    //             $result['result'] = false;
    //         } else {
    //             $this->set_img_offer( $result_ins, $download_offer->logo );
    //             // получаем локальные id категорий по id с leads.su
    //             $id_local_cats = $this->get_conform_cats_id($download_offer->categories);
    //             $result_cat = wp_set_object_terms( $result_ins, $id_local_cats, 'lead_categories', true );
    //             $result['message'] = "{$download_offer->site_name} -- оффер добавлен";
    //         }
    //     }
    //     if (isset($_POST['action'])) {
    //         echo json_encode($result);
    ...
    private function download_all_offers() {
        $answer = file_get_contents("{$this->protocol}//api.leads.su/webmaster/offers/connectedPlatforms?limit=10&platform_id={$this->admin->get_option('platform_id')}&token={$this->admin->get_option('lead_token')}");
        $answer = json_decode($answer, true);
        $this->loading_offers = $answer['data'];
        return $answer;
    }

    private function scan_all_download_id() {
        foreach ($this->loading_offers as $loading_offer) {
            $this->loading_offers_id[] = $loading_offer['id'];
        };
    }

    private function scan_all_local_offers() {
        $this->local_offers = get_posts(array(
            'numberposts'   => -1,
            'post_type'     => 'lead_offers',
            'post_status'   => 'publish',
            'meta_query'    => [
                ...
                'id_lead'   => [
                    'key'   => 'id_lead',
                    'type'  =>'NUMERIC',
                    'meta_value_num' => $this->loading_offers_id,
                    'compare'   => 'IN'
                ]
            ]
        ));

        $this->local_offers_id = [];

        foreach ($this->local_offers as $offer) {
            setup_postdata( $offer );
            $this->local_offers_id[] = get_post_meta($offer->ID, 'id_lead', true);
        }
        wp_reset_postdata();
    }

    private function create_all_new_offers() {
        $result = [];

        $for_create_offers_id = array_diff( $this->loading_offers_id, $this->local_offers_id );

        foreach ($this->loading_offers as $loading_offer) {
            if (! in_array( $loading_offer['id'], $for_create_offers_id )) continue;
            // получаем локальные id категорий по id с leads.su
            $local_ids_category = $this->get_conform_cats_id($loading_offer['categories']);
            // $result['categories_lead'] = $loading_offer['categories'];
            // $result['categories_local'] = $local_ids_category;
            $offer_name = $loading_offer['name'];
            $for_cut = stripos($offer_name,'[');
            if ($for_cut) {
                $offer_name = substr($offer_name, 0, $for_cut);
            }

            $details_stats_system = $loading_offer['detail_stats']['system'];
            
            $offer_data = [
                'post_title'    => $offer_name,
                'post_content'  => $loading_offer['description'],
                'post_type'     => 'lead_offers',
                'post_status'   => 'publish',
                'meta_input'    => [
                    'id_lead'   => $loading_offer['id'],
                    'offer_url' => $loading_offer['offer_url'],
                    'system_other_ctr' => $details_stats_system['other_ctr'],
                    'system_other_cr'  => $details_stats_system['other_cr'],
                    'system_other_ar'  => $details_stats_system['other_ar'],
                    'system_other_epc' => $details_stats_system['other_epc'],
            ...
            $result_ins = wp_insert_post( wp_slash( $offer_data ));
            if( ! is_wp_error($result_ins) ) {
                $result['message'] = 'ID поста: ' . $result_ins;
                $result['new_offers_id'][] = $result_ins;

                echo json_encode($result);
                
                $this->set_img_offer( $result_ins, $loading_offer['logo'] );
                
                // добавим пост к категориям
                $result_cat = wp_set_object_terms( $result_ins, $local_ids_category, 'lead_categories', true );

                if (! is_wp_error($result_cat)) {
                    $result['lead_cats'] = $result_cat;
                } else {
                    $result['message'] = $result_cat->get_error_message();
                    $result['result'] = false;    
                }
            } else {
                $result['message'] = $result_ins->get_error_message();
    ...
    public function create_all_offers() {
        $result = [
            'message'       => '',
            'result'        => true,
            'new_offers_id'   => []
        ...
        $this->download_all_offers();

        $this->scan_all_download_id();

        $this->scan_all_local_offers();

        $result = $this->create_all_new_offers();
    ...
    //     $result = [
    //         'message'   => 'Список платформ успешно получен',
    //         'quest' => "{$this->protocol}//api.leads.su/webmaster/platforms?token={$this->admin->get_option('lead_token')}&status=active",
    //         'result'    => true
    //     ];
    //     $answer = file_get_contents("{$this->protocol}//api.leads.su/webmaster/platforms?token={$this->admin->get_option('lead_token')}");
    //     // $answer = json_decode($answer, true);
    //     echo $answer;
    //     exit;
    //     if ( $answer['code'] == 200 ) {
    //         $this->platforms = $answer['data'];
    //         $result['platforms'] = $this->platforms;
    //         echo json_encode($result);
    //         exit;
    //     } else {
    //         $result['token'] = $this->admin->get_option('lead_token');
    //         $result['message'] = 'Ошибка получения списка платформ.';
