...
add_filter("plugin_action_links_".LEAD_CONST['plugin_basename'] , [$admin, 'lead_plugin_settings_link'] );
...
// Регситрация обработчика fetch запроса на создание офферов
add_action('wp_ajax_create_all_offers', [$offers, 'create_all_offers']);
add_action('wp_ajax_nopriv_create_all_offers', [$offers, 'create_all_offers']);
...
// Регситрация обработчика fetch запроса на обновление оффера
// add_action('wp_ajax_update_offer', [$offers, 'update_offer']);
// add_action('wp_ajax_nopriv_update_offer', [$offers, 'update_offer']);
...
// Регситрация обработчика fetch запроса на gthtrk.xtybt cjcnjzybz jaathf
...
function lead_enqueue_styles( $hook_suffix ) {
    wp_enqueue_style( 'lead_admin', LEAD_CONST['plugin_dir_url'] . 'assets/css/lead_admin.css', array() );
}

add_action('admin_enqueue_scripts', 'lead_enqueue_scripts');
function lead_enqueue_scripts( $hook_suffix ) {
    wp_enqueue_script( 'lead_admin', LEAD_CONST['plugin_dir_url'] . 'assets/js/lead_admin.js', array('jquery') );
}